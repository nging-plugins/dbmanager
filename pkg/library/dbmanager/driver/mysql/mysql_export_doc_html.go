...
func (a *mysqlExportHTMLDoc) Open(c echo.Context) error {
	...
	c.Response().Write([]byte(`<doctype html><html><head><title>Table Documention</title><link rel="stylesheet" href="` + common.BackendURL(c) + `/public/assets/backend/js/bootstrap/dist/css/bootstrap.min.css?t=20220807115920" /></head><body>`))
	return nil
}
...
func (a *mysqlExportHTMLDoc) Close(c echo.Context) error {
	c.Response().Write([]byte(`</body></html>`))
	return nil
}
